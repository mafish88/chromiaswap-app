...
const CommonTokenSelector = ({ slippageTolerance, setSlippageTolerance, children }) => {
						...
						<div className="mx-auto w-full max-w-lg rounded-lg p-5 pt-4 shadow-card dark:bg-light-dark xs:p-6 xs:pt-5">
							<nav className="mb-5 min-h-[40px] border-b border-dashed border-gray-200 pb-4 uppercase dark:border-gray-700 xs:mb-6 xs:pb-5 xs:tracking-wide" style={{ marginTop: '10px' }}>
								<div className="hidden items-center justify-between text-gray-600 dark:text-gray-400 sm:flex">
									<a className={`py-1.5 px-3 font-medium relative ${page === SWAP_TOKEN_PAGE ? 'text-white btn-primary' : ''}`} href="#" style={{ borderRadius: '10px' }} onClick={() => changePage(SWAP_TOKEN_PAGE)}>
										<span>Swap</span>
									...
									<a className={`relative z-[1] inline-flex items-center py-1.5 px-3 ${page === ADD_LIQUIDITY_PAGE ? 'text-white btn-primary' : ''}`} href="#" style={{ borderRadius: '10px' }} onClick={() => changePage(ADD_LIQUIDITY_PAGE)}>
										<span>Liquidity</span>
									</a>
									...
									<SlippageToleranceModal show={showModal} onClose={closeModal} callback={(slippage) => setSlippageTolerance(slippage)} />
								</div>
							</nav>
							{children}
						</div>

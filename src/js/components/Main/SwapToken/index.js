...
const SwapToken = () => {
	...
	const getCirculatingValue = async () => {
		console.log(await blockchain.query("ft3.get_price", {
			_first: secondToken?.id,
			_second: firstToken?.id
		}))

		console.log(await blockchain.query("ft3.get_price", {
			_first: firstToken?.id,
			_second: secondToken?.id
		}))
	}
	useEffect(() => {
		if (chromia_account) {
		...
		<CommonTokenSelector>
			<div data-projection-id="12" style={{ opacity: 1, transform: 'none' }}>
					...
					<div className="relative flex gap-3 flex-col mt-16">
						<div className="tokenSelector group flex min-h-[70px] rounded-lg border border-gray-200 transition-colors duration-200 hover:border-gray-900 dark:border-gray-700 dark:hover:border-gray-600">
							...
							<div className="flex flex-1 flex-col text-right">
								<input type="text" placeholder="0.0" inputMode="decimal"
				...
				<div className="flex flex-col gap-4 xs:gap-[18px]">
					<div className="flex items-center justify-between dark:text-gray-300"><span className="font-medium">Min. Received</span><span>_ _</span></div>
					<div className="flex items-center justify-between dark:text-gray-300"><span className="font-medium">Rate</span><span>_ _</span></div>
					...
					<div className="flex items-center justify-between dark:text-gray-300"><span className="font-medium">Price Slippage</span><span>1%</span></div>

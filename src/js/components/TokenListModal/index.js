const TokenListModal = ({
	...
	const listItems = tokenList.map((token, index) => {
		const isSelected = token?.asset?.name === firstToken?.name || token?.asset?.name === secondToken?.name
		return (
			<li key={index} tabIndex={index}
				className={`flex cursor-pointer items-center gap-2 py-3 px-6 outline-none hover:bg-gray-100 focus:bg-gray-200 dark:hover:bg-gray-800 dark:focus:bg-gray-900 ${isSelected ? "selected-token" : ""}`}
				onClick={() => {
					if (!isSelected) {
						callback(token?.asset)
						onClose()
					}
				}}>
				<div className="token-list">
					<span className="uppercase" style={{ width: "45%", marginRight: "10px", wordBreak: 'break-word' }}>{token?.asset?.name}</span>
					<span style={{ width: "50%", wordBreak: 'break-word', textAlign: 'end' }}>{token?.amount}</span>
				</div>
			</li>
		)
	})
	return (
		<div className="modal fade show " id="exampleModal" tabIndex="-1" aria-labelledby="exampleModalLabel" style={{ display: 'block' }} aria-modal="true" role="dialog">
			<div className="modal-dialog modal-dialog-centered">
				<div className="modal-content">
					<div className="modal-header">
						<h4 className="modals-title  mb-0" id="exampleModalLabel">Swap With</h4>
					...
					<div className="modal-body">
						<div className="relative">
							...
							<input type="search" placeholder="Search Your Coin by Name" className="w-full search-input border-y border-x-0 border-dashed border-gray-200 py-3.5 pl-14 pr-6 text-sm focus:border-gray-300 focus:ring-0 dark:border-gray-700 dark:bg-light-dark-1 focus:dark:border-gray-600" />
						</div>
